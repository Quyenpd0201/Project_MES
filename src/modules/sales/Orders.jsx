  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
      {PRODUCT_SPECS.map((spec) => {
        ...
        if (spec.kind === "num") {
          const { num } = splitNU(get(spec.name)); return (
            <label key={spec.name}>{lbl}
...
function OrderForm({ lookups, editId, copyId, onBack, onSaved, onPrint, onCreateDelivery }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      onSaved();
    } catch (e) { alert("Lưu đơn hàng thất bại: " + e.message); }
  };

  const del = async () => {
    if (!confirm("Xóa đơn hàng này?")) return;
    try {
      await ordersApi.remove(editId);
      alert("Đã xóa đơn hàng thành công!");
      onSaved();
    } catch (e) { alert("Xóa đơn hàng thất bại: " + e.message); }
  };
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-12 gap-3">
                      ...
                      <div className="md:col-span-2">
                        ...
                        <select className={inputCls} disabled={fdis("items")} value={it.unit} onChange={(e) => upItem(it._k, "unit", e.target.value)}>
                          <option value="">-- Chọn --</option>
